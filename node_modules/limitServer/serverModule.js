...
class ModuleServer extends Server{
	...
	promiseRender(){
		...
		return new Promise((resolve, reject) => {
			try{
				Promise.resolve( new me.module( handle.getParam(), me.sqlModule ) ).then((mainObj) => {
					resolve(mainObj);
				}, (errMsg) => {
					reject(errMsg);
				});
	...
	getSqlModule(){
		...
		let sqlModule;
		try{
			sqlModule = require(`${limit.rootPath}${me.getSqlPath()}`);
		}catch(e){
			sqlModule = {};
		};
		['query', 'insertData', 'begin', 'rollback', 'commit'].forEach((val) => {
			sqlModule[val] = SQL[val].bind(SQL);
		});
		return sqlModule;
	}
};
